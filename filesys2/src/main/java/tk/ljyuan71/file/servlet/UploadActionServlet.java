...
public class UploadActionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String sysId = request.getParameter("filesys");
		sysId = StringUtils.isBlank(sysId) ? "defsys" : sysId;
		...
		if ("mergeChunks".equals(action)) {//合并指定临时文件夹下的指定文件(如果存在的话)
			...
			if (fileArray != null && fileArray.length > 0) {
				...
				if (isFull) {
					...
					SysFile addFile = new SysFile(fileName, sysId, true);
					addFile.setFileId(fileId);
					addFile.setMd5(FileSysUtils.getFileMD5(saveFile));
			...
			System.out.println("合并花费："+(System.currentTimeMillis()-st));
