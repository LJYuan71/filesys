	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (StringUtils.isNotBlank(fileId)) {
				...
				if (sysFile != null) {
					path = FileSysUtils.parsePath(path, sysFile.getMd5(), sysFile.getRealTime());
					file = FileSysUtils.getFile(new File(path), sysFile.getMd5()+""+sysFile.getExt(), sysFile.getLength().longValue());//文件
				...
				for (SysFile sysFile : fileList) {
					String tmpPath = FileSysUtils.parsePath(path, sysFile.getMd5(), sysFile.getRealTime());
					file = FileSysUtils.getFile(new File(tmpPath), sysFile.getMd5()+""+sysFile.getExt(), sysFile.getLength().longValue());//文件
					ZipUtils.putAndReadFile(file,zipOut,sysFile.getTitle());
					isExistFile = true;
				}
				
			} else if (StringUtils.isNotBlank(groupPid)) {
				List<SysFile> fileList = sysFileService.queryFileByGroupPid(groupPid);
				response.addHeader("Content-Disposition", "attachment;filename="+ URLEncoder.encode("打包下载"
				+ new SimpleDateFormat("yyyyMMdd").format(new Date())+".zip", "UTF-8"));
				for (SysFile sysFile : fileList) {
					String tmpPath = FileSysUtils.parsePath(path, sysFile.getMd5(), sysFile.getRealTime());
