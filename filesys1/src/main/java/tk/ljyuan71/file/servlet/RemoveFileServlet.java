		...
		try {
			...
			if (StringUtils.isNotBlank(fileId)) {
				...
				if (sysFile != null && userId.equals(sysFile.getUserId()) && sysFile.getCanDel() == 1 && sysFile.getStatus() == 1) {
					...
					File file = FileSysUtils.getFile(new File(path), sysFile.getMd5()+""+sysFile.getExt(),
							sysFile.getLength().longValue());//文件
					if (ls) {
						file.delete();
						sysFileService.deleteById(fileId);
					} else {
						//判断历史文件目录是否已存在相同已删除文件
						path = FileSysUtils.parsePath(FileConfig.get("lsFilePath"), sysFile.getMd5(), sysFile.getRealTime());
						...
						if (!lsFile.exists() || !lsFile.isFile()) {
							...
							FileUtils.copyFileToDirectory(file, 
									new File(FileSysUtils.parsePath(FileConfig.get("lsFilePath"), sysFile.getMd5(), sysFile.getRealTime())));   
						}
